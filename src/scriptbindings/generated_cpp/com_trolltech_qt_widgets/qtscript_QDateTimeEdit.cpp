...
{
    for (int i = 0; i < 11; ++i) {
        if (qtscript_QDateTimeEdit_Section_values[i] == value)
            return QString::fromLatin1(qtscript_QDateTimeEdit_Section_keys[i]);
    }
    return QString();
}
...
static QScriptValue qtscript_construct_QDateTimeEdit_Section(QScriptContext *context, QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 11; ++i) {
        if (qtscript_QDateTimeEdit_Section_values[i] == arg)
