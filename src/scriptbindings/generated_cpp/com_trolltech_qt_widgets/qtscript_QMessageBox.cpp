...
{
    if ((value >= QMessageBox::NoIcon) && (value <= QMessageBox::Question))
        return qtscript_QMessageBox_Icon_keys[static_cast<int>(value)-static_cast<int>(QMessageBox::NoIcon)];
    return QString();
}
...
static QScriptValue qtscript_construct_QMessageBox_Icon(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QMessageBox::NoIcon) && (arg <= QMessageBox::Question))
