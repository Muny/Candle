...
{
    if ((value >= QAbstractSpinBox::CorrectToPreviousValue) && (value <= QAbstractSpinBox::CorrectToNearestValue))
        return qtscript_QAbstractSpinBox_CorrectionMode_keys[static_cast<int>(value)-static_cast<int>(QAbstractSpinBox::CorrectToPreviousValue)];
    return QString();
}
...
static QScriptValue qtscript_construct_QAbstractSpinBox_CorrectionMode(QScriptContext *context, QScriptEngine *engine)
{
    ...
    if ((arg >= QAbstractSpinBox::CorrectToPreviousValue) && (arg <= QAbstractSpinBox::CorrectToNearestValue))
...
{
    if ((value >= QAbstractSpinBox::UpDownArrows) && (value <= QAbstractSpinBox::NoButtons))
        return qtscript_QAbstractSpinBox_ButtonSymbols_keys[static_cast<int>(value)-static_cast<int>(QAbstractSpinBox::UpDownArrows)];
    return QString();
}
...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= QAbstractSpinBox::UpDownArrows) && (arg <= QAbstractSpinBox::NoButtons))
...
static void qtscript_QAbstractSpinBox_StepEnabled_fromScriptValue(const QScriptValue &value, QAbstractSpinBox::StepEnabled &out)
{
    ...
    if (var.userType() == qMetaTypeId<QAbstractSpinBox::StepEnabled>())
        ...
        out = QAbstractSpinBox::StepEnabled();
}
