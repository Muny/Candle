...
{
    for (int i = 0; i < 6; ++i) {
        if (qtscript_QFontDialog_FontDialogOption_values[i] == value)
            return QString::fromLatin1(qtscript_QFontDialog_FontDialogOption_keys[i]);
    }
    return QString();
}
...
static QScriptValue qtscript_construct_QFontDialog_FontDialogOption(QScriptContext *context, QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 6; ++i) {
        if (qtscript_QFontDialog_FontDialogOption_values[i] == arg)
