...
static const char * const qtscript_QMainWindow_function_signatures[] = {
    ...
    , "QToolBar toolbar"
    , "QToolBar before"
    , "List docks, List sizes, Orientation orientation"
    , "QDockWidget dockwidget"
...
{
    for (int i = 0; i < 6; ++i) {
        if (qtscript_QMainWindow_DockOption_values[i] == value)
            return QString::fromLatin1(qtscript_QMainWindow_DockOption_keys[i]);
    }
    return QString();
}
...
static QScriptValue qtscript_construct_QMainWindow_DockOption(QScriptContext *context, QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 6; ++i) {
        if (qtscript_QMainWindow_DockOption_values[i] == arg)
...
static QScriptValue qtscript_QMainWindow_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    case 15:
    if (context->argumentCount() == 3) {
        QList<QDockWidget*> _q_arg0;
        qScriptValueToSequence(context->argument(0), _q_arg0);
        QList<int> _q_arg1;
        qScriptValueToSequence(context->argument(1), _q_arg1);
        Qt::Orientation _q_arg2 = qscriptvalue_cast<Qt::Orientation>(context->argument(2));
        _q_self->resizeDocks(_q_arg0, _q_arg1, _q_arg2);
        return context->engine()->undefinedValue();
    }
    break;
